...
const PREVIEW_BUYERS: Buyer[] = [
  { id: 'preview-prop-house', company_name: 'Northbank Prop House', contact_name: 'Example buyer', email: 'buyer@example.co.uk', phone: null, postcode: 'N1', buyer_type: 'Prop hire / scenic reseller', tags: ['PROP-BIG', 'SCENIC', 'BAR'], status: 'prospect', notes: 'Preview row only.', created_at: new Date().toISOString() },
  { id: 'preview-festival-operator', company_name: 'Fieldline Events Group', contact_name: 'Example buyer', email: 'events@example.co.uk', phone: null, postcode: 'M3', buyer_type: 'Festival / event operator', tags: ['FESTIVAL', 'OUTDOOR', 'DJ-BOOTH'], status: 'prospect', notes: 'Preview row only.', created_at: new Date().toISOString() },
  { id: 'preview-immersive-retail', company_name: 'Arcade Retail Studio', contact_name: 'Example buyer', email: 'studio@example.co.uk', phone: null, postcode: 'SE1', buyer_type: 'Retail / immersive install', tags: ['IMMERSIVE', 'RETAIL', 'PHOTO-MOMENT'], status: 'prospect', notes: 'Preview row only.', created_at: new Date().toISOString() }
];
  ...
  return (
    <section className="room-page buyers-room-page">
      ...
      <div className="room-steps">
        ...
        <div className="room-step"><span>02</span><strong>Verify</strong><small>real contact</small></div>
        ...
        <div className="card room-form-card buyer-form-card">
          ...
          <form action={preloadProspectBuyersAction} className="preload-inline">
            <div>
              ...
              <small>Loads the identified buyer companies using safe placeholder emails.</small>
            </div>
          ...
          <form action={addBuyerAction} className="form room-form buyer-form">
            ...
            <div className="room-two">
              ...
              <label>Postcode<input name="postcode" placeholder="Area" /></label>
            </div>
        ...
        <div className="card room-list-card buyer-list-card">
          <div className="section-head compact buyer-list-head">
            <div>
              ...
              <p className="buyer-section-copy">Click any live buyer to open the record, verify details and refine tags.</p>
            </div>
          ...
          <div className="room-list">
            {buyers.map((buyer) => (
              <Link className="room-list-row buyer-room-row" href={`/admin/buyers/${buyer.id}`} key={buyer.id}>
                <div>
                  ...
                  <small>{buyer.contact_name || 'No contact name yet'} · {buyer.email}</small>
                </div>
            ...
            {previewRows.map((buyer) => (
              <div className="room-list-row buyer-room-row buyer-row-preview" key={buyer.id}>
                <div><strong>{buyer.company_name}</strong><small>{buyer.contact_name} · {buyer.email}</small></div>
                <div className="room-row-meta"><span className="buyer-status buyer-status-prospect">prospect</span><span>{buyer.buyer_type}</span></div>
